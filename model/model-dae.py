...
def parse_args():
    ...
    parser.add_argument('--use_dae', action='store_true', help='Use Denoising Autoencoder')
    parser.add_argument('--dae_hidden_dim', type=int, default=256, help='Hidden dimension for DAE')
    parser.add_argument('--dae_weight', type=float, default=0.3, help='Weight for DAE reconstruction loss')
...
# 模型文件存储目录
root_path = rf'/home/user79/model_train_0622'
...
class DenoisingAutoencoder(nn.Module):
    def __init__(self, input_dim, hidden_dim=256, dropout_rate=0.4):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(hidden_dim // 2, hidden_dim),  # 修复后的代码
            nn.GELU(),
...
class PredictModel(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.use_dae = args.use_dae  # 控制是否使用DAE
        self.factor_num = args.factor_num  # 原始特征维度
        
        # 新增：当use_dae为True时初始化DAE
        if self.use_dae:
            self.dae = DenoisingAutoencoder(
                input_dim=args.factor_num,
                hidden_dim=args.dae_hidden_dim,
                dropout_rate=args.dropout_rate
            )
        
        # 根据是否使用DAE确定输入到filter_layers的特征维度
        if self.use_dae:
            # DAE编码后的特征维度为 hidden_dim // 2（默认256//2=128）
            input_dim = args.dae_hidden_dim // 2
        else:
            # 不使用DAE时，使用原始特征维度
            input_dim = self.factor_num
        
        layer_size_list = [256, 128, 64, 32, 8, 1]  # 各层输出维度
        
        # 特征过滤层（关键修改：使用动态计算的input_dim）
        self.filter_layers = nn.Sequential(
            nn.Linear(input_dim, layer_size_list[0]),
            nn.LeakyReLU(),
        ...
        self.residual_block1 = ResidualBlock(
            in_dim=layer_size_list[0],
            out_dim=layer_size_list[1],
            dropout_rate=args.dropout_rate
        )
        
        # 第一层注意力机制
        self.attention1 = MultiHeadSelfAttention(
            embed_dim=layer_size_list[1],
            num_heads=16,
            dropout=args.dropout_rate
        )
        self.norm1 = nn.LayerNorm(layer_size_list[1])
        self.dropout1 = nn.Dropout(args.dropout_rate) if args.dropout else nn.Identity()
        
        # 第二层注意力前的线性变换
        self.linear1 = nn.Sequential(
            nn.Linear(layer_size_list[1], layer_size_list[2]),
            nn.GELU(),
            nn.Dropout(p=args.dropout_rate) if args.dropout else nn.Identity(),
        )
        
        # 第二层注意力机制
        self.attention2 = MultiHeadSelfAttention(
            embed_dim=layer_size_list[2],
            num_heads=16,
            dropout=args.dropout_rate
        )
        self.norm2 = nn.LayerNorm(layer_size_list[2])
        self.dropout2 = nn.Dropout(args.dropout_rate) if args.dropout else nn.Identity()
        
        # 后续处理层
        self.linear2 = nn.Sequential(
            nn.Linear(layer_size_list[2], layer_size_list[3]),
            nn.GELU(),
            nn.Dropout(p=args.dropout_rate) if args.dropout else nn.Identity(),
        )
        
        self.residual_block2 = ResidualBlock(
            in_dim=layer_size_list[3],
            out_dim=layer_size_list[3],
            dropout_rate=args.dropout_rate
        )
        
        self.output_layers = nn.Sequential(
            nn.Linear(layer_size_list[3], layer_size_list[4]),
            nn.GELU(),
            nn.Dropout(p=args.dropout_rate) if args.dropout else nn.Identity(),
            nn.Linear(layer_size_list[4], layer_size_list[5]),
        )
    ...
    def forward(self, tsdata):
        ...
        # 使用DAE降噪并获取编码特征
        if self.use_dae:
            _, x = self.dae(x)  # 只取编码后的特征
            
            # 调试用：打印DAE后的特征维度
            # print(f"After DAE encoding, feature shape: {x.shape}")

        out = self.filter_layers(x)
        # 调试用：打印过滤层后的特征维度
        # print(f"After filter layers, feature shape: {out.shape}")
        
        out = self.residual_block1(out)
        
        # 第一层注意力
        out1 = out.unsqueeze(0)  # 添加批次维度
        out1 = self.attention1(out1)
        out1 = out1.squeeze(0)  # 移除批次维度
        out = out + self.dropout1(out1)
        out = self.norm1(out)
        ...
        # 第二层注意力
        out2 = out.unsqueeze(0)  # 添加批次维度
        out2 = self.attention2(out2)
        out2 = out2.squeeze(0)  # 移除批次维度
...
class DLLitModule(LightningModule):
    def __init__(self, args):
        ...
        self.recon_loss = nn.MSELoss()  # 新增重构损失
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(len(tsdatas)):
            ...
            ret = ret.unsqueeze(1)
            if self.args.use_dae:
                # Step 1: 直接将原始数据（2000维）输入完整模型，由模型内部处理DAE编码
                preds = self.model(tsdata)  # tsdata → DAE编码 → 预测头 → preds
                
                # Step 2: 复用模型内的DAE，计算重构损失（输入仍是原始2000维数据）
                reconstructed, encoded = self.model.dae(tsdata)  # 正确：输入2000维
                recon_loss = self.recon_loss(reconstructed, tsdata)
                
                # 总损失 = 预测损失 + DAE权重×重构损失
                total_loss += loss_fn(preds, ret, liquid) + self.args.dae_weight * recon_loss
            else:
                # 原训练逻辑保持不变
                preds = self.model(tsdata)
                total_loss += loss_fn(preds, ret, liquid)

        total_loss /= len(tsdatas)
