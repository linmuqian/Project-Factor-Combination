...
# 模型文件存储目录
root_path = rf'/home/user79/model_train_0414'
...
class PredictModel(nn.Module):
    def __init__(self, args):
        super().__init__()
        # 调整 layer_size_list 为 [256, 128, 64, 32, 8, 1]
        layer_size_list = [256, 128, 64, 32, 8, 1]
        
        # 第一层 filter layer 保持不变
        ...
        self.residual_block1 = ResidualBlock(
            in_dim=layer_size_list[0],     # 256
            out_dim=layer_size_list[1],    # 128
            dropout_rate=args.dropout_rate
        )
        
        # 线性层 (128→64)
        self.linear1 = nn.Sequential(
            nn.Linear(layer_size_list[1], layer_size_list[2]),  # 128 → 64
            nn.GELU(),
            nn.Dropout(p=args.dropout_rate) if args.dropout else nn.Identity(),
        )
        ...
        self.linear2 = nn.Sequential(
            nn.Linear(layer_size_list[2], layer_size_list[3]),  # 64 → 32
            nn.GELU(),
            nn.Dropout(p=args.dropout_rate) if args.dropout else nn.Identity(),
        )
        ...
        self.residual_block2 = ResidualBlock(
            in_dim=layer_size_list[3],     # 32
            out_dim=layer_size_list[3],    # 32 (维度不变)
            dropout_rate=args.dropout_rate
        )
        
        # 输出层 (32→8→1)
        self.output_layers = nn.Sequential(
            nn.Linear(layer_size_list[3], layer_size_list[4]),  # 32 → 8
            nn.GELU(),
            nn.Dropout(p=args.dropout_rate) if args.dropout else nn.Identity(),
            
            nn.Linear(layer_size_list[4], layer_size_list[5]),  # 8 → 1 (输出层)
        )
        

    def forward(self, tsdata):
        tsdata = tsdata.float()
        out = self.filter_layers(tsdata)        # factor_num → 256
        out = self.residual_block1(out)         # 256 → 128 (残差连接)
        out = self.linear1(out)                 # 128 → 64
        out = self.linear2(out)                 # 64 → 32
        out = self.residual_block2(out)         # 32 → 32 (残差连接，维度不变)
        out = self.output_layers(out)           # 32 → 8 → 1
        return out
...
class DLLitModule(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # 使用余弦退火学习率调度器
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.args.max_epochs, eta_min=self.args.lr * 0.001)
